...
function DemographicsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="liquid-glass-strong glass-specular-edge border border-emerald-500/40 rounded-2xl p-5 flex items-start gap-4 shadow-[0_12px_35px_rgba(0,0,0,0.4)] relative overflow-hidden">
        <div className="absolute -top-12 -left-12 w-44 h-44 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none" />

        <div className="w-11 h-11 rounded-xl liquid-glass border border-emerald-500/40 flex items-center justify-center text-2xl shrink-0 shadow-[0_0_15px_rgba(78,222,163,0.25)] relative z-10">
          🛡️
        </div>
        <div className="flex-1 relative z-10">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <p className="text-[#4edea3] font-bold text-xs font-mono uppercase tracking-wider drop-shadow-[0_0_6px_rgba(78,222,163,0.3)]">
              Verified Compliance: Aggregated · Anonymized · Inferred · Zero PII
            </p>
            <span className="text-[10px] px-2.5 py-0.5 rounded-lg liquid-glass text-[#4edea3] border border-emerald-500/40 font-mono font-bold shadow-[0_0_8px_rgba(78,222,163,0.2)]">
              DPDP / GDPR COMPLIANT
            </span>
          </div>
      ...
      {loading ? (
        ...
        <>
              ...
              <div className="flex items-center gap-1.5 font-mono text-xs">
                {['country', 'state', 'city'].map(level => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl uppercase transition-all border cursor-pointer hover:scale-102 active:scale-95 ${
                      geoLevel === level
                        ? 'liquid-glass text-[#4cd7f6] border-cyan-400/50 font-bold shadow-[0_0_12px_rgba(76,215,246,0.3)]'
                        : 'liquid-glass-subtle text-[#8ea0b5] border-white/10 hover:text-white hover:border-white/25'
                    }`}
                  >
              ...
              {geoLevel === 'state' && (
                <div className="space-y-3">
                  {data?.top_states?.map((st, i) => (
                    <div key={i} className="p-3.5 liquid-glass-subtle border border-white/10 hover:border-cyan-400/40 rounded-xl flex items-center justify-between gap-3 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(0,0,0,0.4)]">
                      <div className="flex items-center gap-3">
                        <span className="w-7 h-7 rounded-xl liquid-glass text-[#4cd7f6] border border-cyan-400/30 font-mono font-bold text-xs flex items-center justify-center shadow-[0_0_8px_rgba(76,215,246,0.2)]">
                          {i + 1}
                        </span>
                      ...
                      <div className="flex items-center gap-4 text-xs font-mono">
                        ...
                        <span className="text-[#4edea3] font-bold drop-shadow-[0_0_4px_rgba(78,222,163,0.3)]">{st.percentage}%</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {geoLevel === 'city' && (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {data?.top_cities?.map((city, i) => (
                    <div key={i} className="p-4 liquid-glass-subtle border border-white/10 hover:border-cyan-400/40 rounded-xl transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(0,0,0,0.4)]">
                      <div className="text-white font-bold text-sm mb-1 font-sans">{city.name}</div>
                      <div className="text-[11px] font-mono text-[#4cd7f6] font-semibold">{city.count.toLocaleString()} active nodes</div>
          ...
          <div className="liquid-glass-strong glass-specular-edge border border-purple-500/25 rounded-2xl p-6 shadow-[0_15px_45px_rgba(0,0,0,0.5)] relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4 relative z-10">
              ...
              <span className="text-[11px] font-mono text-[#ddb7ff] px-2.5 py-1 rounded-lg liquid-glass border border-purple-500/30 font-bold shadow-[0_0_8px_rgba(221,183,255,0.2)]">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 relative z-10">
              {data?.top_languages?.map((lang) => (
                <div key={lang.language} className="p-3.5 liquid-glass-subtle border border-white/10 hover:border-purple-400/40 rounded-xl text-center transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(0,0,0,0.4)]">
                  <div className="text-xs font-mono font-bold text-white mb-1 truncate">{lang.language}</div>
                  <div className="text-lg font-black font-mono text-[#ddb7ff] drop-shadow-[0_0_6px_rgba(221,183,255,0.3)]">{lang.percentage}%</div>
                  <div className="text-[10px] font-mono text-[#8ea0b5] mt-0.5">{lang.count.toLocaleString()} msgs</div>
                  <div className="text-[10px] font-mono text-[#4edea3] font-bold mt-1 drop-shadow-[0_0_4px_rgba(78,222,163,0.3)]">▲ {lang.trend}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="liquid-glass-strong glass-specular-edge border border-emerald-500/25 rounded-2xl p-6 shadow-[0_15px_45px_rgba(0,0,0,0.5)] relative overflow-hidden">
            <div className="absolute -bottom-20 -right-20 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none" />

            <div className="flex items-center justify-between mb-4 relative z-10">
              <div>
                <h3 className="text-white font-bold text-sm tracking-wider uppercase flex items-center gap-2.5">
                  <span className="w-2.5 h-2.5 rounded-full bg-[#4edea3] shadow-[0_0_10px_#4edea3]" />
              ...
              <span className="text-[11px] font-mono text-[#4edea3] px-2.5 py-1 rounded-lg liquid-glass border border-emerald-500/30 font-bold shadow-[0_0_8px_rgba(78,222,163,0.2)]">
                8 DOMAINS ACTIVE
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 relative z-10">
              {data?.professional_clusters?.map((cluster) => (
                <div
                  key={cluster.domain}
                  className="liquid-glass-interactive rounded-xl p-4 border border-white/10 hover:border-emerald-400/40 transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_15px_30px_rgba(0,0,0,0.5)] group"
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-2xl group-hover:scale-110 transition-transform">{cluster.icon}</span>
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded-lg liquid-glass text-[#4cd7f6] border border-cyan-400/25 font-bold">
                      {cluster.share}
                    </span>
                  </div>
                  <div className="text-white font-bold text-sm mb-1 group-hover:text-[#4edea3] transition-colors">{cluster.domain}</div>
                  <div className="text-xs font-mono text-[#8ea0b5]">
                    {cluster.count.toLocaleString()} cohort signals
                  </div>
                  <div className="flex items-center justify-between text-[10px] font-mono text-[#8ea0b5] border-t border-white/10 pt-2 mt-2">
                    <span>Affinity: <strong className="text-white">{cluster.affinity}</strong></span>
