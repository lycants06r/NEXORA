  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 font-mono">
        <div className="liquid-glass border border-rose-500/30 rounded-2xl p-4 flex items-center justify-between hover:border-rose-500/50 transition-all shadow-[0_4px_20px_rgba(244,63,94,0.1)] relative overflow-hidden group">
        ...
        <div className="liquid-glass border border-amber-500/30 rounded-2xl p-4 flex items-center justify-between hover:border-amber-500/50 transition-all shadow-[0_4px_20px_rgba(245,158,11,0.1)] relative overflow-hidden group">
          <div className="glass-edge-top" />
          <div className="relative z-10">
        ...
        <div className="liquid-glass border border-cyan-500/30 rounded-2xl p-4 flex items-center justify-between hover:border-cyan-500/50 transition-all shadow-[0_4px_20px_rgba(76,215,246,0.1)] relative overflow-hidden group">
          <div className="glass-edge-top" />
          <div className="relative z-10">
          ...
          <span className="text-3xl relative z-10">ℹ️</span>
        </div>
      </div>
          ...
          {['ALL', 'CRITICAL', 'WARNING', 'INFO'].map(sev => (
            <button
              ...
              className={`h-8 px-3 rounded-xl uppercase tracking-wider transition-all cursor-pointer font-semibold ${
                severityFilter === sev
                  ? 'bg-[rgba(76,215,246,0.18)] text-[#4cd7f6] border border-[#4cd7f6]/40 font-bold shadow-glow-cyan backdrop-blur-md'
        ...
        <div className="flex items-center gap-1.5">
          ...
          {['ALL', 'ACTIVE', 'INVESTIGATING', 'ACKNOWLEDGED', 'RESOLVED'].map(st => (
            <button
              ...
              className={`h-8 px-3 rounded-xl uppercase tracking-wider transition-all cursor-pointer font-semibold ${
                statusFilter === st
                  ? 'bg-purple-500/25 text-[#ddb7ff] border border-purple-500/50 font-bold shadow-[0_0_12px_rgba(221,183,255,0.25)] backdrop-blur-md'
                  : 'glass-control text-[#8ea0b5] hover:text-white hover:border-white/20'
              }`}
            >
              {st}
      ...
      <div className="space-y-4">
        {filteredAlerts.length === 0 ? (
          <div className="p-12 text-center liquid-glass rounded-2xl border border-white/10 text-[#8ea0b5] font-mono">
          ...
          filteredAlerts.map(alert => (
            <div
              key={alert.alert_id}
              className="liquid-glass rounded-2xl p-5 shadow-glass-card transition-all hover:border-cyan-400/40 relative overflow-hidden group"
            >
              <div className="glass-edge-top" />
              <div className="flex items-start justify-between flex-wrap gap-3 mb-2.5 relative z-10">
                <div className="flex items-center gap-2.5 flex-wrap">
                  ...
                  <span className="text-xs font-mono text-[#8ea0b5] uppercase liquid-glass-soft border border-white/10 px-2 py-0.5 rounded-lg">
                    {alert.category}
                  </span>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg liquid-glass-soft border border-cyan-500/30 text-[#4cd7f6] text-xs font-mono font-semibold uppercase">
              ...
              <h3 className="text-base font-bold text-white mb-1.5 font-sans relative z-10">
                {alert.title}
              </h3>
              <p className="text-xs text-[#dae2fd] leading-relaxed mb-3.5 font-sans relative z-10">
                {alert.description}
              </p>

              {/* Action Plan & Remediation */}
              <div className="p-3.5 liquid-glass-soft rounded-xl border border-white/10 flex items-center justify-between flex-wrap gap-3 text-xs font-mono relative z-10">
                ...
                {alert.status === 'ACTIVE' && (
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="glass-btn-secondary h-8 px-3.5 rounded-lg font-semibold transition-all cursor-pointer flex items-center"
                    >
                      Acknowledge
                    </button>
                    <button
                      ...
                      className="h-8 px-3.5 rounded-lg bg-emerald-500/20 border border-emerald-500/40 hover:bg-emerald-500/30 text-[#4edea3] font-bold transition-all cursor-pointer flex items-center shadow-[0_0_10px_rgba(16,185,129,0.3)]"
                    >
                      Resolve Alert
                    </button>
                  </div>
